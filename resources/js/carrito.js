const addProduct = (id) => {
  const productos = window.localStorage.getItem("carritoPHARMA");
  if (!productos) {
    const newProducts = JSON.stringify([formatProducts({ id })]);
    localStorage.setItem("carritoPHARMA", newProducts);
    numeroItemCart();
    return;
  }
  ...
  if (item) {
    ...
    localStorage.setItem("carritoPHARMA", JSON.stringify(updatedProducts));
    mostrarAlerta();
    numeroItemCart();
    return;
  }
  ...
  localStorage.setItem("carritoPHARMA", JSON.stringify(newProducts));
  mostrarAlerta();
  numeroItemCart();
};
