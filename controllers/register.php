...
if (!empty($_POST["nombre"]) && !empty($_POST["email"]) && !empty($_POST["password"])) {
    ...
    $sqlEmail = "SELECT * FROM `usuarios` WHERE `correo` = '$email'";
    if ($conn->consultar($sqlEmail) == 1) {
        mostrarMensaje("Error", "Este correo ya está registrado, utiliza otro correo", "error", "/pages/register.php");
        exit;
    }

    $sql = "INSERT INTO `usuarios`(`id`, `nombre_completo`, `correo`, `contrasena`, `id_role`) VALUES (NULL,'$nombre', '$email', '$contra', 2)";
